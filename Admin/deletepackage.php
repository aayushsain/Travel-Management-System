<?php
// C:\travel\Admin\deletepackage.php
...
// Check if a package is pre-selected via GET
$selected_id = isset($_GET["pid"]) ? (int)$_GET["pid"] : (isset($_POST["t1"]) ? (int)$_POST["t1"] : 0);
...
if (isset($_POST["sbmt"])) {
    ...
    if ($selected_id <= 0) {
        ...
        $ok = prepare_exec($cn, "DELETE FROM package WHERE Packid = ?", "i", [$selected_id]);
        if ($ok) {
            echo "<script>alert('Package deleted successfully!'); window.location.href='viewpackage.php';</script>";
            exit;
        } else {
...
// Fetch packages list for selection dropdown
...
<head>
    ...
    <title>VoyageQuest - Delete Package</title>
...
<div class="container">
    ...
    <div class="col-sm-9 fade-in-up">
        ...
        <form method="post" onsubmit="return confirm('Are you sure you want to permanently delete this package?');">
            <input type="hidden" name="csrf_token" value="<?php echo generate_csrf_token(); ?>">
            
            <table border="0" width="100%" align="center" class="tableshadow">
                <thead>
                    <tr>
                        <th colspan="2" class="toptd">Delete Package</th>
                    </tr>
                </thead>
                <tbody>
                    <tr>
                        ...
                        <td>
                            <select name="t1" required>
                                <option value="">Select Package to Delete</option>
                                <?php
                                if ($packages_list) {
                    ...
                    <tr>
                        <td>&nbsp;</td>
                        <td>
                            <input type="submit" value="Delete Package" name="sbmt" style="background: linear-gradient(135deg, #EF4444, #b91c1c) !important; box-shadow: 0 4px 15px rgba(239, 68, 68, 0.25) !important;" />
                        </td>
                    </tr>
                </tbody>
            </table>
